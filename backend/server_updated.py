...
async def send_otp(request: OTPRequest):
    """Send OTP to phone number - NO DEMO MODE"""
    try:
...
async def verify_otp(request: OTPVerify):
    """Verify OTP and authenticate user - NO DEMO MODE"""
    try:
        if not twilio_client or not TWILIO_VERIFY_SERVICE_SID:
            raise HTTPException(status_code=500, detail="OTP service not configured")
...
async def create_payment_order(request: PaymentRequest, user_id: str = Depends(verify_jwt_token)):
    """Create Razorpay payment order - NO DEMO MODE"""
    try:
...
async def verify_payment(
    ...
    """Verify Razorpay payment - NO DEMO MODE"""
    try:
        if not razorpay_client:
            raise HTTPException(status_code=500, detail="Payment service not configured")
...
async def broker_signup(broker: BrokerSignup):
    ...
    try:
        ...
        if existing_broker:
            raise HTTPException(status_code=400, detail="Broker already registered")
...
# ADMIN ROUTES - SEPARATE AND SECURED
